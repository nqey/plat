  ...
  <div class="plat-content">
    <div class="plat-content-con">
      ...
      <div class="index_table_btn">
        <button class="btn btn-primary" v-if="selectState && selectState === 'doing'" @click="$refs.sendModal.$refs.modal.toggle();">发送</button>
        <button class="btn btn-primary" v-if="selectState && selectState === 'wait'" data-toggle="modal" data-target="#myModal2" @click="$refs.timeModal.$refs.modal.toggle();">设置时长</button>
        <a class="btn btn-primary" v-if="selectState && selectState !== 'wait'" :href="download + examinationId">下载数据</a>
        <button class="btn btn-primary" @click="copyExam" v-if="examinationId">复制试题</button>
        <button class="btn btn-primary" v-if="selectState && selectState === 'wait'" data-toggle="modal" data-target="#myModal3" @click="$refs.objecModal.$refs.modal.toggle();">考试对象</button>
      </div>
      <!-- 发送modal -->
      <v-send-modal ref="sendModal" :id="examinationId"></v-send-modal>
      <!-- 设置时长modal -->
      <v-time-modal ref="timeModal" :id="examinationId" :item="item"></v-time-modal>
      <!-- 考试对象modal -->
      <v-object-modal ref="objecModal" :id="examinationId" :item="item"></v-object-modal>
      <!-- 答卷modal -->
      <v-test-modal ref="testModal" :id="examinationId" :item="item"></v-test-modal>
    </div>
  </div>
</template>
...
PLATFORM_POST_EXAMS_EXAMINEE_DOWNLOADEXCEL } from '@/config/env';
